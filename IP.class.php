...
class IP {
     ...
     * @return string
     */
    public function generateIP($province = ''){
        $ip_address = $this->_IPSegment($province);
        $ip = $this->_randomIP($ip_address['begin'],$ip_address['end']);
        return $ip;
    }
     ...
     * 更新各省IPV4地址段
     */
    public function updateIPSegment(){
     ...
     * @return string
     */
    private function _randomIP($ip_begin,$ip_end){
     ...
     * @return array|string
     */
    private function _IPSegment($province = ''){
        $ip_segment = [];
     ...
     * @return bool
     */
    private function _write($filename,$values,$var='ip_segment',$format=false){
        $file=$filename;
        if(is_array($values)){
            $text="<?php\r\n".'$'.$var.'='.$this->_arrayConvert($values,$format).";";
        }else{
            $text=$values;
        }
        return $this->_writeFile($file,$text);
    }
     ...
     * @return string
     */
    private function _arrayConvert($array,$format=false,$level=0){
        $space=$line='';
        if(!$format){
            for($i=0;$i<=$level;$i++){
                $space.="\t";
        ...
        foreach($array as $key=> $val){
            ...
            if(is_array($val)){
                $evaluate.=$comma.$key.'=>'.$this->_arrayConvert($val,$format,$level+1);
            }else{
     ...
     * @return bool
     */
    private function _writeFile($filename,$text,$openMode='w'){
